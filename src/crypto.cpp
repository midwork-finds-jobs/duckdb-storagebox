...
const EVP_CIPHER *AESStateSSL::GetCipher(idx_t key_len) {
  ...
  default:
    throw InternalException("Invalid Encryption/Decryption Cipher: %d",
...
void AESStateSSL::InitializeEncryption(const_data_ptr_t iv, idx_t iv_len,
                                       const_data_ptr_t key, idx_t key_len_p,
                                       const_data_ptr_t aad, idx_t aad_len) {
  mode = EncryptionTypes::ENCRYPT;

  if (key_len_p != key_len) {
    throw InternalException(
        "Invalid encryption key length, expected %llu, got %llu", key_len,
        key_len_p);
  }
  ...
  if (1 != EVP_CIPHER_CTX_ctrl(context, EVP_CTRL_GCM_SET_IVLEN, iv_len, NULL)) {
    throw InternalException(
        "EVP_CIPHER_CTX_ctrl failed (EVP_CTRL_GCM_SET_IVLEN)");
  }
...
void AESStateSSL::InitializeDecryption(const_data_ptr_t iv, idx_t iv_len,
  ...
  if (cipher == EncryptionTypes::GCM) {
    if (1 !=
        EVP_CIPHER_CTX_ctrl(context, EVP_CTRL_GCM_SET_IVLEN, iv_len, NULL)) {
...
size_t AESStateSSL::FinalizeGCM(data_ptr_t out, idx_t out_len, data_ptr_t tag,
                                idx_t tag_len) {
  ...
  default:
    throw InternalException("Unhandled encryption mode %d",
