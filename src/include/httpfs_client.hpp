...
struct HTTPFSParams : public HTTPParams {
  ...
  bool enable_server_cert_verification =
      DEFAULT_ENABLE_SERVER_CERT_VERIFICATION;
  ...
  string user_agent = {""};
  // Additional fields needs to be appended at the end and need to be propagated
  // to duckdb-wasm
...
public:
  ...
  InitializeParameters(optional_ptr<FileOpener> opener,
                       optional_ptr<FileOpenerInfo> info) override;
  unique_ptr<HTTPClient>
  InitializeClient(HTTPParams &http_params,
                   const string &proto_host_port) override;
