...
public:
  ...
  static constexpr const char *MESSAGE =
      "Content-Length from server mismatches requested range, server may not "
      "support range requests. You can try to "
      "resolve this by enabling `SET force_download=true`";

  static void Throw() { throw HTTPException(MESSAGE); }
};
...
public:
  ...
  // In write overwrite mode, we are not interested in the current state of the
  // file: we're overwriting it.
...
public:
  static bool TryParseLastModifiedTime(const string &timestamp,
                                       timestamp_t &result);
  ...
  // HTTP Requests
  virtual duckdb::unique_ptr<HTTPResponse>
  HeadRequest(FileHandle &handle, string url, HTTPHeaders header_map);
  // Get Request with range parameter that GETs exactly buffer_out_len bytes
  // from the url
  virtual duckdb::unique_ptr<HTTPResponse>
  ...
  virtual duckdb::unique_ptr<HTTPResponse>
  GetRequest(FileHandle &handle, string url, HTTPHeaders header_map);
  // Post Request that can handle variable sized responses without a
  // content-length header (needed for s3 multipart)
  virtual duckdb::unique_ptr<HTTPResponse>
  PostRequest(FileHandle &handle, string url, HTTPHeaders header_map,
  ...
  virtual duckdb::unique_ptr<HTTPResponse>
  DeleteRequest(FileHandle &handle, string url, HTTPHeaders header_map);
  ...
  string GetName() const override { return "HTTPFileSystem"; }
...
protected:
  ...
  virtual HTTPException GetHTTPError(FileHandle &, const HTTPResponse &response,
                                     const string &url);
  bool TryRangeRequest(FileHandle &handle, string url, HTTPHeaders header_map,
                       idx_t file_offset, char *buffer_out,
                       idx_t buffer_out_len);
  bool ReadInternal(FileHandle &handle, void *buffer, int64_t nr_bytes,
                    idx_t location);

protected:
  virtual duckdb::unique_ptr<HTTPFileHandle>
