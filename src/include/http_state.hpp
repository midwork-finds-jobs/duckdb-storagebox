...
//! Represents a file that is intended to be fully downloaded, then used in
...
private:
  ...
  mutex lock;
  //! When initialized is set to true, the file is safe for parallel reading
...
public:
  ...
  bool Initialized() { return file->initialized; }
  const char *GetData() { return file->data.get(); }
  uint64_t GetCapacity() { return file->capacity; }
...
public:
  ...
  bool IsEmpty() {
    return head_count == 0 && get_count == 0 && put_count == 0 &&
           post_count == 0 && delete_count == 0 && total_bytes_received == 0 &&
           total_bytes_sent == 0;
  }
  ...
  void QueryEnd(ClientContext &context) override { Reset(); }
