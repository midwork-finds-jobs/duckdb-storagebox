...
HTTPFSUtil::InitializeParameters(optional_ptr<FileOpener> opener,
  ...
  // Setting lookups
  FileOpener::TryGetCurrentSetting(opener, "http_timeout", result->timeout,
                                   info);
  FileOpener::TryGetCurrentSetting(opener, "force_download",
                                   result->force_download, info);
  FileOpener::TryGetCurrentSetting(opener, "auto_fallback_to_full_download",
                                   result->auto_fallback_to_full_download,
                                   info);
  FileOpener::TryGetCurrentSetting(opener, "http_retries", result->retries,
                                   info);
  FileOpener::TryGetCurrentSetting(opener, "http_retry_wait_ms",
                                   result->retry_wait_ms, info);
  FileOpener::TryGetCurrentSetting(opener, "http_retry_backoff",
                                   result->retry_backoff, info);
  FileOpener::TryGetCurrentSetting(opener, "http_keep_alive",
                                   result->keep_alive, info);
  FileOpener::TryGetCurrentSetting(
      opener, "enable_curl_server_cert_verification",
  ...
  FileOpener::TryGetCurrentSetting(opener, "enable_server_cert_verification",
                                   ...
                                   info);
  FileOpener::TryGetCurrentSetting(opener, "ca_cert_file", result->ca_cert_file,
                                   info);
  FileOpener::TryGetCurrentSetting(opener, "hf_max_per_page",
                                   result->hf_max_per_page, info);
  FileOpener::TryGetCurrentSetting(opener, "unsafe_disable_etag_checks",
                                   result->unsafe_disable_etag_checks, info);

  {
    auto db = FileOpener::TryGetDatabase(opener);
    if (db) {
      result->user_agent = StringUtil::Format("%s %s", db->config.UserAgent(),
                                              DuckDB::SourceID());
    }
  }
  ...
  if (settings_reader.TryGetSecretKey("extra_http_headers", extra_headers)) {
    ...
    for (const auto &child : children) {
      ...
      D_ASSERT(kv.size() == 2);
      result->extra_headers[kv[0].GetValue<string>()] =
          kv[1].GetValue<string>();
    }
  }
...
static void AddUserAgentIfAvailable(HTTPFSParams &http_params,
...
unique_ptr<HTTPResponse> HTTPFileSystem::PostRequest(
    FileHandle &handle, string url, HTTPHeaders header_map, string &buffer_out,
  ...
  PostRequestInfo post_request(url, header_map, hfh.http_params,
...
HTTPFileSystem::PutRequest(FileHandle &handle, string url,
  ...
  PutRequestInfo put_request(url, header_map, hfh.http_params,
                             (const_data_ptr_t)buffer_in, buffer_in_len,
                             content_type);
  return http_util.Request(put_request);
}

unique_ptr<HTTPResponse> HTTPFileSystem::HeadRequest(FileHandle &handle,
                                                     string url,
                                                     HTTPHeaders header_map) {
  auto &hfh = handle.Cast<HTTPFileHandle>();
  auto &http_util = hfh.http_params.http_util;
...
unique_ptr<HTTPResponse> HTTPFileSystem::DeleteRequest(FileHandle &handle,
                                                       string url,
                                                       HTTPHeaders header_map) {
  auto &hfh = handle.Cast<HTTPFileHandle>();
  auto &http_util = hfh.http_params.http_util;
...
unique_ptr<HTTPResponse> HTTPFileSystem::GetRequest(FileHandle &handle,
                                                    string url,
                                                    HTTPHeaders header_map) {
  auto &hfh = handle.Cast<HTTPFileHandle>();
  auto &http_util = hfh.http_params.http_util;
  ...
  GetRequestInfo get_request(
      ...
      [&](const HTTPResponse &response) {
        if (static_cast<int>(response.status) >= 400) {
          ...
          if (response.status == HTTPStatusCode::RangeNotSatisfiable_416) {
            error +=
                " This could mean the file was changed. Try disabling the "
                "duckdb http metadata cache "
                "if enabled, and confirm the server supports range requests.";
          }
...
unique_ptr<HTTPResponse>
HTTPFileSystem::GetRangeRequest(FileHandle &handle, string url,
  ...
  string range_expr = "bytes=" + to_string(file_offset) + "-" +
                      to_string(file_offset + buffer_out_len - 1);
      ...
      [&](const HTTPResponse &response) {
        ...
        if (static_cast<int>(response.status) < 300) { // done redirecting
          ...
          if (!hfh.http_params.unsafe_disable_etag_checks &&
              !hfh.etag.empty() && response.HasHeader("ETag")) {
            string responseEtag = response.GetHeaderValue("ETag");

            if (!responseEtag.empty() && responseEtag != hfh.etag) {
              throw HTTPException(
                  response,
                  "ETag was initially %s and now it returned %s, this likely "
                  "means the remote file has "
                  "changed.\nTry to restart the read or close the file-handle "
                  "and read the file again (e.g. "
                  "`DETACH` in the file is a database file).\nYou can disable "
                  "checking etags via `SET "
          ...
          if (response.HasHeader("Content-Length")) {
            auto content_length =
                stoll(response.GetHeaderValue("Content-Length"));
      ...
      [&](const_data_ptr_t data, idx_t data_length) {
        if (buffer_out != nullptr) {
          if (data_length + out_offset > buffer_out_len) {
            // As of v0.8.2-dev4424 we might end up here when very big files are
            // served from servers that returns more data than requested via
            // range header. This is an uncommon but legal behaviour, so we have
            // to improve logic elsewhere to properly handle this case.

            // To avoid corruption of memory, we bail out.
            throw HTTPException("Server sent back more data than expected, "
                                "`SET force_download=true` might "
                                "help in this case");
          }
...
HTTPFileHandle::HTTPFileHandle(FileSystem &fs, const OpenFileInfo &file,
                               FileOpenFlags flags,
                               unique_ptr<HTTPParams> params_p)
    : FileHandle(fs, file.path, flags), params(std::move(params_p)),
      http_params(params->Cast<HTTPFSParams>()), flags(flags), length(0),
      force_full_download(false), buffer_available(0), buffer_idx(0),
      file_offset(0), buffer_start(0), buffer_end(0) {
  // check if the handle has extended properties that can be set directly in the
  // handle if we have these properties we don't need to do a head request to
  // obtain them later
  if (file.extended_info) {
    ...
    if (lm_entry != info.end() && etag_entry != info.end() &&
        fs_entry != info.end()) {
...
HTTPFileSystem::CreateHandle(const OpenFileInfo &file, FileOpenFlags flags,
  ...
  if (secret_manager && transaction) {
    auto secret_match =
        secret_manager->LookupSecret(*transaction, file.path, "bearer");

    if (secret_match.HasMatch()) {
      const auto &kv_secret = dynamic_cast<const KeyValueSecret &>(
          *secret_match.secret_entry->secret);
      auto &httpfs_params = params->Cast<HTTPFSParams>();
      httpfs_params.bearer_token =
          kv_secret.TryGetValue("token", true).ToString();
    }
  }
  return duckdb::make_uniq<HTTPFileHandle>(*this, file, flags,
                                           std::move(params));
}

unique_ptr<FileHandle>
HTTPFileSystem::OpenFileExtended(const OpenFileInfo &file, FileOpenFlags flags,
                                 optional_ptr<FileOpener> opener) {
  D_ASSERT(flags.Compression() == FileCompressionType::UNCOMPRESSED);
...
bool HTTPFileSystem::TryRangeRequest(FileHandle &handle, string url,
                                     HTTPHeaders header_map, idx_t file_offset,
                                     char *buffer_out, idx_t buffer_out_len) {
  auto res = GetRangeRequest(handle, url, header_map, file_offset, buffer_out,
                             buffer_out_len);
  ...
  throw IOException("Unknown error for HTTP %s to '%s'",
                    EnumUtil::ToString(RequestType::GET_REQUEST), url);
}

bool HTTPFileSystem::ReadInternal(FileHandle &handle, void *buffer,
  ...
  if (skip_buffer && to_read > 0) {
    if (!TryRangeRequest(hfh, hfh.path, {}, location, (char *)buffer,
                         to_read)) {
      return false;
    }
    DUCKDB_LOG_FILE_SYSTEM_READ(handle, nr_bytes, location);
  ...
  while (to_read > 0) {
    ...
    if (to_read > 0 && hfh.buffer_available == 0) {
      auto new_buffer_available =
          MinValue<idx_t>(hfh.READ_BUFFER_LEN, hfh.length - start_offset);

      // Bypass buffer if we read more than buffer size
      if (to_read > new_buffer_available) {
        if (!TryRangeRequest(hfh, hfh.path, {}, location + buffer_offset,
        ...
        if (!TryRangeRequest(hfh, hfh.path, {}, start_offset,
                             (char *)hfh.read_buffer.get(),
...
void HTTPFileSystem::Read(FileHandle &handle, void *buffer, int64_t nr_bytes,
                          idx_t location) {
  ...
  // ReadInternal returned false. This means the regular path of querying the
  // file with range requests failed. We will attempt to download the full file
  // and retry.

  if (handle.logger) {
    DUCKDB_LOG_WARN(
        handle.logger,
        "Falling back to full file download for file '%s': the server does not "
        "support HTTP range "
  ...
  if (!ReadInternal(handle, buffer, nr_bytes, location)) {
    throw HTTPException("Failed to read from HTTP file after automatically "
                        "retrying a full file download.");
  }
}

int64_t HTTPFileSystem::Read(FileHandle &handle, void *buffer,
                             int64_t nr_bytes) {
  auto &hfh = handle.Cast<HTTPFileHandle>();
...
void HTTPFileSystem::Write(FileHandle &handle, void *buffer, int64_t nr_bytes,
                           idx_t location) {
  throw NotImplementedException("Writing to HTTP files not implemented");
}

int64_t HTTPFileSystem::Write(FileHandle &handle, void *buffer,
                              int64_t nr_bytes) {
  auto &hfh = handle.Cast<HTTPFileHandle>();
...
TryGetMetadataCache(optional_ptr<FileOpener> opener, HTTPFileSystem &httpfs) {
  ...
  if (use_shared_cache) {
    ...
    return client_context->registered_state
        ->GetOrCreate<HTTPMetadataCache>("http_cache", true, true)
        .get();
  }
  return nullptr;
}

void HTTPFileHandle::FullDownload(HTTPFileSystem &hfs,
                                  bool &should_write_cache) {
  // We are going to download the file at full, we don't need to do no head
  // request.
  ...
  if (!cached_file_handle->Initialized()) {
    ...
    if (full_download_result->status != HTTPStatusCode::OK_200) {
      throw HTTPException(*full_download_result,
                          "Full download failed to to URL \"%s\": %d (%s)",
                          full_download_result->url,
                          static_cast<int>(full_download_result->status),
                          full_download_result->GetError());
    }
    // Mark the file as initialized, set its final length, and unlock it to
...
bool HTTPFileSystem::TryParseLastModifiedTime(const string &timestamp,
                                              timestamp_t &result) {
  StrpTimeFormat::ParseResult parse_result;
  if (!StrpTimeFormat::TryParse("%a, %d %h %Y %T %Z", timestamp,
                                parse_result)) {
    return false;
  }
...
optional_idx TryParseContentRange(const HTTPHeaders &headers) {
  ...
  if (range_find == std::string::npos ||
      content_range.size() < range_find + 1) {
    return optional_idx();
  }
...
void HTTPFileHandle::LoadFileInfo() {
  if (initialized || force_full_download) {
    // already initialized or we specifically do not want to perform a head
    // request and just run a direct download
    return;
  }

  // In write_overwrite_mode we dgaf about the size, so no head request is
  // needed
  ...
  if (res->status != HTTPStatusCode::OK_200) {
    if (flags.OpenForWriting() && res->status == HTTPStatusCode::NotFound_404) {
      ...
      // HEAD request fail, use Range request for another try (read only one
      // byte)
      if (flags.OpenForReading() &&
        ...
        if (range_res->status != HTTPStatusCode::PartialContent_206 &&
          ...
          // It failed again
          throw HTTPException(*range_res,
                              "Unable to connect to URL \"%s\": %d (%s).", path,
                              static_cast<int>(res->status), res->GetError());
        }
        res = std::move(range_res);
      } else {
        throw HTTPException(*res, "Unable to connect to URL \"%s\": %d (%s).",
                            res->url, static_cast<int>(res->status),
                            res->GetError());
      }
    }
  }
  ...
  if (res->headers.HasHeader("Last-Modified")) {
    HTTPFileSystem::TryParseLastModifiedTime(
        res->headers.GetHeaderValue("Last-Modified"), last_modified);
  }
  ...
  if (flags.OpenForReading()) {
    ...
    if (current_cache) {
      ...
      if (found) {
        ...
        if (flags.OpenForReading()) {
          read_buffer =
              duckdb::unique_ptr<data_t[]>(new data_t[READ_BUFFER_LEN]);
        }
        return;
      }
...
HTTPFileHandle::~HTTPFileHandle() { DUCKDB_LOG_FILE_SYSTEM_CLOSE((*this)); };

string HTTPFSUtil::GetName() const { return "HTTPFS"; }
