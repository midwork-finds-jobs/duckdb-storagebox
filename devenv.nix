...

{
  ...
  packages = with pkgs; [
    ...
    # For duckdb-webdav extension
    openssl
    curl

    # For duckdb-sshfs extension
    libssh2
  ];
  ...
  git-hooks.hooks = {
    ...
    # Runs tests for both duckdb-webdav and duckdb-sshfs
    unit-tests = {
      enable = true;

      name = "Unit tests";

      # Run tests for both extensions that don't require Docker services
      # - httpfs and secrets tests for duckdb-webdav
      # - sshfs tests for duckdb-sshfs
      entry = "bash -c 'if [ -f build/release/test/unittest ]; then build/release/test/unittest \"[httpfs]\" \"[secrets]\" \"[sshfs]\" || true; else echo \"Tests not built yet, skipping...\"; fi'";
